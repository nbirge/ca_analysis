...
if col ==4:
    with PdfPages('./testing/residuals-pulser.pdf') as pdf:
        for i in np.linspace(0,18,19,dtype=int):
            ...
            plt.ylabel('30 mV Pulse Fitted Peak Width (ADC)',fontsize=rsize)
...
#plt.imshow((z[:,1:]),interpolation='none',origin='lower',cmap=cm.coolwarm,vmin=10,vmax=50)
...
if col == 4:
    ...
    nm='-chisq'
plt.savefig('./testing/pulser-contour'+nm,bbox_inches='tight',format='pdf')
plt.close()
